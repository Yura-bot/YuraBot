...
class AntiSpamClient extends EventEmitter {
	...
	constructor (options) {
		...
		this.options = {
			...
			ignoreBots: options.ignoreBots != undefined ? options.ignoreBots : false,
			...
			removeBotMessages: options.removeBotMessages || true,
	...
	async clearSpamMessages (messages, client) {
		try {
			messages.forEach((message) => {
				...
				if (channel) {
					const msg = channel.messages.cache.get(message.messageID).catch(e => {return})
					if (msg && msg.deletable) msg.delete().catch(e => {return})
				}
			})
		} catch (e) {
			if(e){
	...
	async banUser (message, member, spamMessages) {
		...
		if (!member.bannable) {
			...
			if (this.options.errorMessages) {
				let send = message.channel.send(this.format(message.language("EVENTS_MESSAGE_ANTIRAID_BAN"), message)).catch((e) => {
					if (this.options.verbose) {
        ...
        if (this.options.errorMessages) {
          message.channel.send(this.format(message.language("EVENTS_MESSAGE_ANTIRAID_BAN"), message)).catch((e) => {
            if (this.options.verbose) {
	...
	async muteUser (message, member, spamMessages) {
		...
		const role = message.guild.roles.cache.get(message.muteRole)
		...
		if (this.options.muteMessage) {
			await message.channel.send(this.format(message.language("EVENTS_MESSAGE_ANTIRAID_MUTE"), message)).catch(e => {
				if (this.options.verbose) {
	...
	async kickUser (message, member, spamMessages) {
		...
		if (!member.kickable) {
			...
			if (this.options.kickMessage) {
				message.channel.send(this.format(message.language("EVENTS_MESSAGE_ANTIRAID_KICK"), message)).catch((e) => {
					if (this.options.verbose) {
						console.error(`DAntiSpam (kickUser#sendSuccessMessage): ${e.message}`)
					}
				}).then(msg => {
	...
	async warnUser (message, member, spamMessages) {
		...
		if (this.options.warnMessage) {
			message.channel.send(this.format(message.language("EVENTS_MESSAGE_ANTIRAID_WARN"), message)).catch((e) => {
				if (this.options.verbose) {
	 ...
	 * });
	 */
	async message (message, client, db) {

		let guildLanguage = !db.lang ? "english": db.lang;
		
		message.language = require(`../../languages/${guildLanguage}`);
		message.muteRole = db.muteRole
