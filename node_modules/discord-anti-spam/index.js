	...
	async message(message, client, db) {
		...
		const banUser = async () => {
			...
			try {
				...
				if (options.banMessage)
					await message.channel.send(format(language("EVENTS_MESSAGE_ANTIRAID_BAN"), message)).catch(e => {
						if (options.verbose) console.error(e);
					});
		...
		const kickUser = async () => {
			...
			if (!message.member.kickable) {
				...
				if (options.errorMessages)
					await message.channel
						.send(format(language("EVENTS_MESSAGE_ANTIRAID_KICK"), message))
						.catch((e) => {
							if (options.verbose) console.error(e);
						});
				return false;
			...
			try {
				...
				if (options.kickMessage)
					await message.channel.send(format(language("EVENTS_MESSAGE_ANTIRAID_KICK"), message)).catch(e => {
						if (options.verbose) console.error(e);
					});
		...
		const warnUser = async () => {
			...
			if (options.warnMessage)
				await message.channel.send(format(language("EVENTS_MESSAGE_ANTIRAID_WARN"), message)).catch(e => {
					if (options.verbose) console.error(e);
				});
